...
void startFile(int amount, int inputs, int outputs, bool clear){
	std::fstream file;
...
void logFile(const FloatVector& input, const FloatVector& output, bool clear){
	static int n = 0;
	...
	if (n == 1)
		...
		startFile(n, input.size(), output.size(), clear);
